...
class UserServiceTest {
    ...
    fun `getUserById returns the profile when it exists`() {
        val user = User(id = 3L, cognitoId = "sub-3", name = "Pedro", email = "pedro@puce.edu", phone = null)
    ...
    @Test
    fun `getUserById lanza UserNotFoundException cuando el id no existe`() {
    ...
    @Test
    fun `updateUser lanza UserNotFoundException cuando el perfil no existe`() {
        val request = UserRequest(name = "Nuevo", email = "nuevo@puce.edu", phone = null)
        `when`(userRepository.findByCognitoId(cognitoId)).thenReturn(Optional.empty())
    ...
    fun `updateUser lanza BlankNameException cuando el nombre esta vacio`() {
        val existing = User(id = 7L, cognitoId = cognitoId, name = "Viejo", email = "viejo@puce.edu", phone = null)
        val request = UserRequest(name = "  ", email = "x@puce.edu", phone = null)
        `when`(userRepository.findByCognitoId(cognitoId)).thenReturn(Optional.of(existing))
    ...
    fun `deleteUser elimina el usuario cuando existe`() {
        ...
        org.mockito.Mockito.verify(userRepository).deleteById(5L)
    }
}
